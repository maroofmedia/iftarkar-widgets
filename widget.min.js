document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchData() {
    const cacheKey = "prayerTimingsData";
    ...
    if (cachedData) {
      ...
      if (Date.now() - timestamp < cacheExpiry) {
        ...
        return Promise.resolve(data); // Use cached data directly
      }
    }

    // Fetch fresh data from the API
    console.log("Fetching fresh data from API");
    return fetch("https://jkmeeqat-api.pages.dev/data.json")
      .then((response) => {
        if (!response.ok) {
          throw new Error("Failed to fetch prayer timings data");
        }
  ...
  // Fetch data and populate the widget
  fetchData()
    .then((data) => {
